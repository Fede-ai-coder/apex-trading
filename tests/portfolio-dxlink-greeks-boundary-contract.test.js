...
eq(LIVE_INDEX.length, PORTFOLIO_SPY_PRICE_U.EXTRACTED_CHARS,
...
eq(sha256(LIVE_INDEX), PORTFOLIO_SPY_PRICE_U.EXTRACTED_SHA256, '…and its digest');
eq(APP_LOADER.parseScriptTags(LIVE_INDEX).filter((t) => t.src && /^\.\//.test(t.src)).length,
  LOCAL_SCRIPT_COUNT + 15, '…carrying one more local script for every layer cut since');
{
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  eq(changed.filter((rel) => rel === 'index.html' || rel.startsWith('js/')),
    ['index.html', 'js/config/strategy-templates.js', 'js/portfolio/portfolio-vega-monitor.js',
      ...
      'js/ui/chart-interactions.js', 'js/services/journal-snapshot-helpers.js', MODULE_REL, 'js/services/journal-map-audit.js', 'js/services/dxlink-greeks-fetch.js', 'js/portfolio/portfolio-technical-parity.js', 'js/portfolio/portfolio-spy-price.js'].sort(),
