...
eq(PART_RANGES.indexOf(adapterPart[0]), PART_RANGES.length - 44,
   'ORDER: the adapter sits that many parts from the tail — the distance grows with every layer');
eq(PART_RANGES.indexOf(previewPart[0]), PART_RANGES.length - 43,
   ...
   'ORDER: the adapter/preview adjacency is measured, not just their absolute slots');
{
  ...
  eq(PART_RANGES.indexOf(ttReconnectPart[0]), PART_RANGES.length - 22,
     'ORDER: the TT reconnect owner precedes every layer cut after it — the gap is counted off\n     PART_RANGES rather than listed here, because it grows by one each cycle');
