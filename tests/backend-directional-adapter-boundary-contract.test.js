...
eq(PART_RANGES.indexOf(adapterPart[0]), PART_RANGES.length - 49,
   'ORDER: the adapter sits that many parts from the tail — the distance grows with every layer');
eq(PART_RANGES.indexOf(previewPart[0]), PART_RANGES.length - 48,
   ...
   'ORDER: the adapter/preview adjacency is measured, not just their absolute slots');
{
  ...
  eq(PART_RANGES.indexOf(ttReconnectPart[0]), PART_RANGES.length - 27,
