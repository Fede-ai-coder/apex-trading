...
// occupant is named, so an unplanned script appearing in that gap still fails.
{
  ...
  const journalRemoteTagIdx = SCRIPT_TAGS.findIndex(function (t) { return /journal-remote-persistence\.js$/.test(String(t.src || '')); });
  ...
  const journalWriteThroughTagIdx = SCRIPT_TAGS.findIndex(function (t) { return /journal-backend-write-through\.js$/.test(String(t.src || '')); });
  ...
  const journalManualImportTagIdx = SCRIPT_TAGS.findIndex(function (t) { return /journal-manual-import\.js$/.test(String(t.src || '')); });
...
eq(PART_RANGES.indexOf(adapterPart[0]), PART_RANGES.length - 42,
   'ORDER: the adapter sits that many parts from the tail — the distance grows with every layer');
eq(PART_RANGES.indexOf(previewPart[0]), PART_RANGES.length - 41,
   ...
   'ORDER: the adapter/preview adjacency is measured, not just their absolute slots');
{
  ...
  const mcxPart = PART_RANGES.filter(function (r) { return /mcx-market-context\.js$/.test(r.src); });
  ...
  const mcxBackendPart = PART_RANGES.filter(function (r) { return /mcx-backend-candles\.js$/.test(r.src); });
  ...
  const journalRemotePart = PART_RANGES.filter(function (r) { return /journal-remote-persistence\.js$/.test(r.src); });
  eq(journalRemotePart.length, 1, 'ORDER: the Journal Remote owner is present exactly once');
  eq(PART_RANGES.indexOf(journalUiPart[0]), PART_RANGES.indexOf(journalRemotePart[0]) - 1,
     'ORDER: Journal UI is immediately before Journal Remote');
  const journalWriteThroughPart = PART_RANGES.filter(function (r) { return /journal-backend-write-through\.js$/.test(r.src); });
  ...
  const journalManualImportPart = PART_RANGES.filter(function (r) { return /journal-manual-import\.js$/.test(r.src); });
  ...
  eq(PART_RANGES.indexOf(ttReconnectPart[0]), PART_RANGES.length - 20,
