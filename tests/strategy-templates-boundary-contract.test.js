...
eq(LIVE_INDEX.length, BACKEND_POSITIONS_AGGREGATE_U.EXTRACTED_CHARS,
...
eq(sha256(LIVE_INDEX), BACKEND_POSITIONS_AGGREGATE_U.EXTRACTED_SHA256, '…and its digest');
eq(APP_LOADER.parseScriptTags(LIVE_INDEX).filter((t) => t.src && /^\.\//.test(t.src)).length,
  LOCAL_SCRIPT_COUNT + 8, '…carrying one more local script for every layer cut since');
{
...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  eq(changed.filter((rel) => rel === 'index.html' || rel.startsWith('js/')),
    ['index.html', 'js/portfolio/portfolio-vega-monitor.js', 'js/services/scanner-earnings-throttle.js',
      'js/services/scanner-ivr-throttle.js', 'js/portfolio/backend-positions-aggregate.js', 'js/services/swing-direction.js', 'js/services/swing-weekly-candles.js', 'js/ui/chart-interactions.js',
