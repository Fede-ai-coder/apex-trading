...
eq(A.localSrcs[A.localSrcs.length - 20], './js/portfolio/backend-portfolios.js',
...
eq(A.localSrcs[A.localSrcs.length - 19], './js/portfolio/portfolio-expiry-manual.js',
...
eq(A.localSrcs[A.localSrcs.length - 18], './js/portfolio/portfolio-traffic-light.js',
...
eq(A.localSrcs[A.localSrcs.length - 17], './js/ui/backend-candle-store-chart.js',
...
eq(A.localSrcs[A.localSrcs.length - 16], './js/services/journal-rich-snapshot.js',
...
// slot is pinned by nothing unless a NEW final clause is added with it.
eq(A.localSrcs[A.localSrcs.length - 15], './js/portfolio/portfolio-backend-candles.js',
...
eq(A.localSrcs[A.localSrcs.length - 14], './js/services/journal-snapshot-prefetch.js',
...
// last slot is pinned by nothing unless a NEW final clause is added with it.
eq(A.localSrcs[A.localSrcs.length - 13], './js/portfolio/portfolio-dxlink-greeks.js',
...
eq(A.localSrcs[A.localSrcs.length - 12], './js/config/strategy-templates.js',
  '9.11z6 the strategy-templates owner is immediately before the vega-monitor owner');
// Re-terminated with the chain: bumping the indices alone would leave the last
// slot asserted by nothing at all.
eq(A.localSrcs[A.localSrcs.length - 11], './js/portfolio/portfolio-vega-monitor.js',
...
eq(A.localSrcs[A.localSrcs.length - 10], './js/services/scanner-ivr-throttle.js',
  '9.11z8 the scanner-IVR owner is immediately before the scanner-Earnings owner');
eq(A.localSrcs[A.localSrcs.length - 9], './js/services/scanner-earnings-throttle.js',
...
eq(A.localSrcs[A.localSrcs.length - 8], './js/ui/chart-interactions.js',
  '9.11z10 the chart-interactions owner is immediately before the snapshot-helper owner');
eq(A.localSrcs[A.localSrcs.length - 7], './js/services/journal-snapshot-helpers.js',
...
eq(A.localSrcs[A.localSrcs.length - 6], './js/services/swing-weekly-candles.js',
  '9.11z12 the swing weekly-candle owner is immediately before the swing-direction owner');
// Re-terminated with the chain once more: every link above moved down one, so
// the new last slot gets its own clause instead of inheriting the "newest"
// claim from the layer that no longer holds it.
eq(A.localSrcs[A.localSrcs.length - 5], './js/services/swing-direction.js',
  '9.11z13 the swing direction resolver is immediately before the backend positions aggregate');
// Re-terminated with the chain once more: every link above moved down one, so
// the new last slot gets its own clause instead of inheriting the "newest"
// claim from the layer that no longer holds it.
eq(A.localSrcs[A.localSrcs.length - 4], './js/portfolio/backend-positions-aggregate.js',
  '9.11z14 the backend positions aggregate is immediately before the portfolio technical merge');
// Re-terminated with the chain once more: every link above moved down one, so
// the new last slot gets its own clause instead of inheriting the "newest"
// claim from the layer that no longer holds it.
eq(A.localSrcs[A.localSrcs.length - 3], './js/portfolio/portfolio-technical-merge.js',
  '9.11z15 the portfolio technical merge is immediately before the alignment debug pair');
// Re-terminated with the chain once more: every link above moved down one, so
// the new last slot gets its own clause instead of inheriting the "newest"
// claim from the layer that no longer holds it.
eq(A.localSrcs[A.localSrcs.length - 2], './js/portfolio/portfolio-technical-alignment-debug.js',
  '9.11z16 the alignment debug pair is immediately before the journal map-audit owner');
// Re-terminated with the chain once more: every link above moved down one, so
// the new last slot gets its own clause instead of inheriting the "newest"
// claim from the layer that no longer holds it.
eq(A.localSrcs[A.localSrcs.length - 1], './js/services/journal-map-audit.js',
  '9.11z17 the journal map-audit owner is the newest local script before the monolith');
...
const GUARDS = [
  ...
  ['dsb-tail-preserved', (r) => r.localSrcs[r.localSrcs.length - 43] === './js/ui/backend-directional-snapshot-panel.js' &&
    r.localSrcs[r.localSrcs.length - 42] === './js/services/pretrade-risk-rules.js' &&
    r.localSrcs[r.localSrcs.length - 41] === './js/services/pretrade-technicals.js' &&
    r.localSrcs[r.localSrcs.length - 40] === './js/ui/pretrade-risk-modal.js' &&
    r.localSrcs[r.localSrcs.length - 39] === './js/services/mcx-market-context.js' &&
    r.localSrcs[r.localSrcs.length - 38] === './js/services/mcx-vix-market-context.js' &&
    r.localSrcs[r.localSrcs.length - 37] === './js/services/mcx-backend-candles.js' &&
    r.localSrcs[r.localSrcs.length - 36] === './js/services/journal-core.js' &&
    r.localSrcs[r.localSrcs.length - 35] === './js/services/mcx-regime-policy.js' &&
    r.localSrcs[r.localSrcs.length - 34] === './js/ui/journal-ui.js' &&
    r.localSrcs[r.localSrcs.length - 33] === './' + JOURNAL_REMOTE_REL &&
    r.localSrcs[r.localSrcs.length - 32] === './' + JOURNAL_WRITE_THROUGH_REL &&
    r.localSrcs[r.localSrcs.length - 31] === './' + JOURNAL_MIGRATION_REL &&
    r.localSrcs[r.localSrcs.length - 30] === './' + JOURNAL_MANUAL_IMPORT_REL &&
    r.localSrcs[r.localSrcs.length - 29] === './js/ui/journal-backup-restore.js' &&
    r.localSrcs[r.localSrcs.length - 28] === './js/ui/mcx-macro-check.js' &&
    r.localSrcs[r.localSrcs.length - 27] === './js/ui/mcx-charts.js' &&
    r.localSrcs[r.localSrcs.length - 26] === './js/services/apex-post-auth-init.js' &&
    r.localSrcs[r.localSrcs.length - 25] === './js/ui/tt-reconnect.js' &&
    r.localSrcs[r.localSrcs.length - 24] === './js/ui/journal-close-legs.js' &&
    r.localSrcs[r.localSrcs.length - 23] === './js/ui/journal-trade-forms.js' &&
    r.localSrcs[r.localSrcs.length - 22] === './js/ui/journal-trade-detail.js' &&
    r.localSrcs[r.localSrcs.length - 21] === './js/portfolio/portfolio-data-fetch.js' &&
    r.localSrcs[r.localSrcs.length - 20] === './js/portfolio/backend-portfolios.js' &&
    r.localSrcs[r.localSrcs.length - 19] === './js/portfolio/portfolio-expiry-manual.js' &&
    r.localSrcs[r.localSrcs.length - 18] === './js/portfolio/portfolio-traffic-light.js' &&
    ...
    r.localSrcs[r.localSrcs.length - 17] === './js/ui/backend-candle-store-chart.js' &&
    r.localSrcs[r.localSrcs.length - 16] === './js/services/journal-rich-snapshot.js' &&
    r.localSrcs[r.localSrcs.length - 15] === './js/portfolio/portfolio-backend-candles.js' &&
    r.localSrcs[r.localSrcs.length - 14] === './js/services/journal-snapshot-prefetch.js' &&
    r.localSrcs[r.localSrcs.length - 13] === './js/portfolio/portfolio-dxlink-greeks.js' &&
    r.localSrcs[r.localSrcs.length - 12] === './js/config/strategy-templates.js' &&
    r.localSrcs[r.localSrcs.length - 11] === './js/portfolio/portfolio-vega-monitor.js' &&
    r.localSrcs[r.localSrcs.length - 10] === './js/services/scanner-ivr-throttle.js' &&
    r.localSrcs[r.localSrcs.length - 9] === './js/services/scanner-earnings-throttle.js' &&
    r.localSrcs[r.localSrcs.length - 8] === './js/ui/chart-interactions.js' &&
    r.localSrcs[r.localSrcs.length - 7] === './js/services/journal-snapshot-helpers.js' &&
    ...
    r.localSrcs[r.localSrcs.length - 6] === './js/services/swing-weekly-candles.js' &&
    r.localSrcs[r.localSrcs.length - 5] === './js/services/swing-direction.js' &&
    r.localSrcs[r.localSrcs.length - 4] === './js/portfolio/backend-positions-aggregate.js' &&
    r.localSrcs[r.localSrcs.length - 3] === './js/portfolio/portfolio-technical-merge.js' &&
    r.localSrcs[r.localSrcs.length - 2] === './js/portfolio/portfolio-technical-alignment-debug.js' &&
    // Re-terminated with the chain again: the last slot gets a clause of its
    // own rather than inheriting one from the layer that moved off it.
    r.localSrcs[r.localSrcs.length - 1] === './js/services/journal-map-audit.js'],
