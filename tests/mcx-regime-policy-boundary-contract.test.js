...
const journalAt = INDEX.indexOf(JOURNAL_TAG), regimeAt = INDEX.indexOf(REGIME_TAG), inlineAt = INDEX.indexOf(INLINE_OPEN);
eq(count(INDEX, REGIME_TAG), 1, 'exactly one MCX Regime Policy script tag');
eq(INDEX.slice(mcx1At, inlineAt),
  MCX1_TAG + '\n' + MCX2_TAG + '\n' + MCX3_TAG + '\n' + JOURNAL_TAG + '\n' + REGIME_TAG + '\n',
  'service tail is contiguous MCX1 -> MCX2 -> MCX3 -> Journal -> Regime Policy -> inline');
ok(mcx1At >= 0 && mcx2At > mcx1At && mcx3At > mcx2At && journalAt > mcx3At && regimeAt > journalAt && inlineAt > regimeAt,
  'Regime Policy loads synchronously after Journal and before residual inline code');
...
const rebuilt = U.undoMcxRegimePolicy(INDEX, MODULE);
...
throws(() => U.undoMcxRegimePolicy(INDEX, MODULE + ' '), 'module-byte mutant is rejected');
throws(() => U.undoMcxRegimePolicy(INDEX.replace(REGIME_TAG, REGIME_TAG + '\n' + REGIME_TAG), MODULE),
  'duplicate-tag mutant is rejected');
throws(() => U.undoMcxRegimePolicy(INDEX.replace(REGIME_TAG, ''), MODULE), 'missing-tag mutant is rejected');
