...
expectOk(() => verifyLoad(SCRIPT_MODEL), '4.1 the script tag and its slot satisfy every load predicate');
{
  ...
  eq(local.length, 84,
     '4.9 index.html loads that many local application scripts; the tail chain below names the newest of them');
  ...
  });
  {
    ...
    eq(slots[slots.length - 1] + 1, local.length - 32,
       '4.8g and the run ends immediately before the MCX macro-check owner');
  }
  eq(local[local.length - 32].src, './js/ui/mcx-macro-check.js',
  ...
  eq(local[local.length - 31].src, './js/ui/mcx-charts.js',
     '4.9c the MCX charts owner is immediately before the Apex shared post-auth owner');
  eq(local[local.length - 30].src, './js/services/apex-post-auth-init.js',
  ...
  eq(local[local.length - 29].src, './js/ui/tt-reconnect.js',
  ...
  eq(local[local.length - 28].src, './js/ui/journal-close-legs.js',
  ...
  eq(local[local.length - 27].src, './js/ui/journal-trade-forms.js',
  ...
  eq(local[local.length - 26].src, './js/ui/journal-trade-detail.js',
  ...
  eq(local[local.length - 25].src, './js/portfolio/portfolio-data-fetch.js',
  ...
  eq(local[local.length - 24].src, './js/portfolio/backend-portfolios.js',
  ...
  eq(local[local.length - 23].src, './js/portfolio/portfolio-expiry-manual.js',
  ...
  eq(local[local.length - 22].src, './js/portfolio/portfolio-traffic-light.js',
  ...
  eq(local[local.length - 21].src, './js/ui/backend-candle-store-chart.js',
  ...
  eq(local[local.length - 20].src, './js/services/journal-rich-snapshot.js',
  ...
  eq(local[local.length - 19].src, './js/portfolio/portfolio-backend-candles.js',
  ...
  eq(local[local.length - 18].src, './js/services/journal-snapshot-prefetch.js',
  ...
  eq(local[local.length - 17].src, './js/portfolio/portfolio-dxlink-greeks.js',
  ...
  eq(local[local.length - 16].src, './js/config/strategy-templates.js',
  ...
  // Re-terminated with the chain: without this clause the last slot would be
  // pinned by nothing. (The ordinal this comment used to carry — "a tenth time,
  ...
  eq(local[local.length - 15].src, './js/portfolio/portfolio-vega-monitor.js',
     '4.9k23 the vega-monitor owner is immediately before the scanner-IVR owner');
  ...
  eq(local[local.length - 14].src, './js/services/scanner-ivr-throttle.js',
     '4.9k25 the scanner-IVR owner is immediately before the scanner-Earnings owner');
  ...
  eq(local[local.length - 13].src, './js/services/scanner-earnings-throttle.js',
     '4.9k27 the scanner-Earnings owner is immediately before the chart-interactions owner');
  ...
  eq(local[local.length - 12].src, './js/ui/chart-interactions.js',
     '4.9k29 the chart-interactions owner is immediately before the snapshot-helper owner');
  // RE-TERMINATED with the chain, for the same reason as every clause above it:
  // each link moved down one, so the last slot needs a clause of its own or the
  // endpoint is pinned by nothing at all — the defect CLAUDE.md records this
  // file for, and the reason a shifted chain must GAIN an assertion.
  eq(local[local.length - 11].src, './js/services/journal-snapshot-helpers.js',
     '4.9k31 the snapshot-helper owner is immediately before the swing weekly-candle owner');
  // RE-TERMINATED AGAIN, in the file that gave this rule its name: the clause
  // above used to claim the LAST slot, and shifting the chain would have left
  // that claim one place too high while the true endpoint went unasserted.
  eq(local[local.length - 10].src, './js/services/swing-weekly-candles.js',
     '4.9k33 the swing weekly-candle owner is immediately before the swing-direction owner');
  eq(local.filter((s) => s.src === './js/services/swing-weekly-candles.js').length, 1,
     '4.9k34 exactly one swing weekly-candle tag, no duplicate');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above used to claim the LAST slot, and shifting the chain alone
  // would have left that claim one place too high while the true endpoint went
  // unasserted — which is exactly the defect CLAUDE.md records here.
  eq(local[local.length - 9].src, './js/services/swing-direction.js',
     '4.9k35 the swing direction resolver is immediately before the backend positions aggregate');
  eq(local.filter((s) => s.src === './js/services/swing-direction.js').length, 1,
     '4.9k36 exactly one swing direction tag, no duplicate');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above held the endpoint claim, and shifting alone would have left it
  // one place too high while the true endpoint went unasserted.
  eq(local[local.length - 8].src, './js/portfolio/backend-positions-aggregate.js',
     '4.9k37 the backend positions aggregate is immediately before the portfolio technical merge');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above held the endpoint claim, and shifting alone would have left it
  // one place too high while the true endpoint went unasserted.
  eq(local[local.length - 7].src, './js/portfolio/portfolio-technical-merge.js',
     '4.9k39 the portfolio technical merge is immediately before the alignment debug pair');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above held the endpoint claim, and shifting alone would have left it
  // one place too high while the true endpoint went unasserted.
  eq(local[local.length - 6].src, './js/portfolio/portfolio-technical-alignment-debug.js',
     '4.9k41 the alignment debug pair is immediately before the journal map-audit owner');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above held the endpoint claim, and shifting alone would have left it
  // one place too high while the true endpoint went unasserted.
  eq(local[local.length - 5].src, './js/services/journal-map-audit.js',
     '4.9k43 the journal map-audit owner is immediately before the DXLink greeks-fetch owner');
  eq(local.filter((s) => s.src === './js/services/journal-map-audit.js').length, 1,
     '4.9k44 exactly one journal map-audit tag, no duplicate');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above held the endpoint claim, and shifting alone would have left it
  // one place too high while the true endpoint went unasserted.
  eq(local[local.length - 4].src, './js/services/dxlink-greeks-fetch.js',
     '4.9k45 the DXLink greeks-fetch owner is immediately before the technical-parity owner');
  eq(local.filter((s) => s.src === './js/services/dxlink-greeks-fetch.js').length, 1,
     '4.9k46 exactly one DXLink greeks-fetch tag, no duplicate');
  // RE-TERMINATED ONCE MORE, in the file that gave this rule its name: the
  // clause above held the endpoint claim, and shifting alone would have left it
  // one place too high while the true endpoint went unasserted.
  eq(local[local.length - 3].src, './js/portfolio/portfolio-technical-parity.js',
     '4.9k47 the technical-parity owner is the THIRD-last local application script — the\n'
     + '     wording is re-terminated with the index, because "LAST" was left standing here\n'
     + '     through two cycles that had already made it false');
  eq(local[local.length - 2].src, './js/portfolio/portfolio-spy-price.js',
     '4.9k the portfolio SPY price owner of #468 is the second-last local script');
  eq(local[local.length - 1].src, './js/services/apex-storage-recovery.js',
     '4.9k49 the storage-recovery owner of #470 is the NEWEST local script, read off the tail\n'
     + '     by POSITION so the endpoint is not pinned by an index alone');
  eq(local.filter((s) => s.src === './js/portfolio/portfolio-technical-parity.js').length, 1,
     '4.9k48 exactly one technical-parity tag, no duplicate');
  eq(local.filter((s) => s.src === './js/portfolio/portfolio-technical-alignment-debug.js').length, 1,
     '4.9k42 exactly one alignment debug tag, no duplicate');
  eq(local.filter((s) => s.src === './js/portfolio/portfolio-technical-merge.js').length, 1,
     '4.9k40 exactly one portfolio technical merge tag, no duplicate');
  eq(local.filter((s) => s.src === './js/portfolio/backend-positions-aggregate.js').length, 1,
     '4.9k38 exactly one backend positions aggregate tag, no duplicate');
  eq(local.filter((s) => s.src === './js/services/journal-snapshot-helpers.js').length, 1,
     '4.9k32 exactly one snapshot-helper tag, no duplicate');
