...
expectOk(() => verifyLoad(SCRIPT_MODEL), '4.1 the script tag and its slot satisfy every load predicate');
{
  ...
  eq(local.length, 57,
     '4.9 index.html loads 57 local application scripts, including PRETRADE, all four MCX owners, seven Journal owners and the MCX macro-check, MCX charts, Apex post-auth, TT reconnect and Journal Close Legs owners');
  eq(local[local.length - 5].src, './js/ui/mcx-macro-check.js',
  ...
  eq(local[local.length - 4].src, './js/ui/mcx-charts.js',
     '4.9c the MCX charts owner is immediately before the Apex shared post-auth owner');
  eq(local[local.length - 3].src, './js/services/apex-post-auth-init.js',
  ...
  eq(local[local.length - 2].src, './js/ui/tt-reconnect.js',
  ...
  eq(local[local.length - 1].src, './js/ui/journal-close-legs.js',
     '4.9h the Journal Close Legs owner is the LAST local application script before the inline monolith');
