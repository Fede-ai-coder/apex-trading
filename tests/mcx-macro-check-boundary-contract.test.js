...
eq(LIVE_INDEX.length, 1496195, 'the live shipped index UTF-16 length is the newest layer’s extracted value');
eq(sha256(LIVE_INDEX), 'cad874aeadb31e613487488a54ab9bcd92043bf8478084f55c148638dc1df900',
...
eq(APP_LOADER.parseScriptTags(LIVE_INDEX).filter((entry) => entry.src && /^\.\//.test(entry.src)).length, 80,
  'the current shipped index carries exactly 80 local application scripts');
