  ...
  deepEq(PARTS.filter(function (p) {
    return p.kind === 'local' && /(^|\/)js\/ui\//.test(String(p.src == null ? '' : p.src));
     ...
     [BSS_PANEL_REL, DSB_PANEL_REL, PREVIEW_REL, SFS_PANEL_REL, PESS_BATCH_PANEL_REL, PESS_UI_PANEL_REL,
      ...
      './js/ui/tt-reconnect.js', './js/ui/journal-close-legs.js'].slice().sort(),
     'SCOPE: js/ui/ contributes exactly sixteen named scripts, including the later Journal UI, Backup/Restore, MCX macro-check, MCX charts, TT reconnect and Journal Close Legs owners');
