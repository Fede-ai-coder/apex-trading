...
(function () {
  ...
  eq(iTtReconnect, srcs.length - 11,
...
(function () {
  ...
  eq(ttReconnectTagIdx, inlineTagIdx - 10,
  ...
  deepEq(PARTS.filter(function (p) {
    return p.kind === 'local' && /(^|\/)js\/ui\//.test(String(p.src == null ? '' : p.src));
     ...
     [BSS_PANEL_REL, DSB_PANEL_REL, PREVIEW_REL, SFS_PANEL_REL, PESS_BATCH_PANEL_REL, PESS_UI_PANEL_REL,
      ...
      './js/ui/backend-candle-store-chart.js'].slice().sort(),
     'SCOPE: js/ui/ contributes exactly nineteen named scripts, including the later Journal UI, Backup/Restore, MCX macro-check, MCX charts, TT reconnect, Journal Close Legs and trade-forms owners');
