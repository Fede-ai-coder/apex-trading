...
(function () {
  ...
  const iMcxVix = srcs.indexOf('./js/services/mcx-vix-market-context.js');
  const iMcxBackend = srcs.indexOf('./js/services/mcx-backend-candles.js');
  ok(iMcxVix >= 0, 'index.html loads the newest MCX VIX market-context script');
  eq(iMcx, iMcxVix - 1, 'ORDER: the MCX market-context owner is immediately before the MCX VIX owner');
  eq(iMcxVix, iMcxBackend - 1, 'ORDER: the MCX VIX owner is immediately before the MCX backend-candle owner');
  ...
  const iJournalRemote = srcs.indexOf(JOURNAL_REMOTE_REL);
  ok(iJournalRemote >= 0, 'index.html loads the Journal Remote script');
  eq(iJournalUi, iJournalRemote - 1, 'ORDER: Journal UI is immediately before Journal Remote');
  const iJournalWriteThrough = srcs.indexOf(JOURNAL_WRITE_THROUGH_REL);
  ...
  const iJournalManualImport = srcs.indexOf(JOURNAL_MANUAL_IMPORT_REL);
  ...
  const iMcxMacroCheck = srcs.indexOf(MCX_MACRO_CHECK_REL);
  ok(iMcxMacroCheck >= 0, 'index.html loads the MCX macro-check script');
  eq(iJournalBackupRestore, iMcxMacroCheck - 1,
     'ORDER: Journal Backup/Restore is immediately before the MCX macro-check owner');
  const iMcxCharts = srcs.indexOf(MCX_CHARTS_REL);
  ...
  eq(iTtReconnect, srcs.length - 19,
     'ORDER: the TT reconnect owner precedes the Journal Close Legs, trade-forms, trade-detail, portfolio and traffic-light owners');
...
(function () {
  ...
  const mcxVixTagIdx = TAGS.findIndex(function (t) { return /mcx-vix-market-context\.js$/.test(clean(t.src)); });
  const mcxBackendTagIdx = TAGS.findIndex(function (t) { return /mcx-backend-candles\.js$/.test(clean(t.src)); });
  ok(mcxVixTagIdx >= 0, 'tag order: the newest MCX VIX market-context tag is present');
  eq(mcxTagIdx, mcxVixTagIdx - 1, 'tag order: the MCX market-context owner remains immediately before the MCX VIX owner');
  ok(mcxBackendTagIdx >= 0, 'tag order: the MCX backend-candle owner is present');
  eq(mcxVixTagIdx, mcxBackendTagIdx - 1, 'tag order: the MCX VIX owner remains immediately before the MCX backend-candle owner');
  ...
  const journalRemoteTagIdx = TAGS.findIndex(function (t) { return /journal-remote-persistence\.js$/.test(clean(t.src)); });
  ok(journalRemoteTagIdx >= 0, 'tag order: the Journal Remote owner is present');
  eq(journalUiTagIdx, journalRemoteTagIdx - 1, 'tag order: Journal UI is immediately before Journal Remote');
  const journalWriteThroughTagIdx = TAGS.findIndex(function (t) { return /journal-backend-write-through\.js$/.test(clean(t.src)); });
  ...
  const journalManualImportTagIdx = TAGS.findIndex(function (t) { return /journal-manual-import\.js$/.test(clean(t.src)); });
  ...
  const mcxMacroCheckTagIdx = TAGS.findIndex(function (t) { return /mcx-macro-check\.js$/.test(clean(t.src)); });
  ...
  const mcxChartsTagIdx = TAGS.findIndex(function (t) { return /mcx-charts\.js$/.test(clean(t.src)); });
  ...
  eq(ttReconnectTagIdx, inlineTagIdx - 18,
     'tag order: the TT reconnect owner precedes the Journal Close Legs, trade-forms, trade-detail,\n' +
     '     portfolio data-fetch, backend-portfolios, manual-expiry and traffic-light owners');
  ...
  deepEq(PARTS.filter(function (p) {
    return p.kind === 'local' && /(^|\/)js\/ui\//.test(String(p.src == null ? '' : p.src));
     ...
     [BSS_PANEL_REL, DSB_PANEL_REL, PREVIEW_REL, SFS_PANEL_REL, PESS_BATCH_PANEL_REL, PESS_UI_PANEL_REL,
      ...
      './js/ui/tt-reconnect.js', './js/ui/journal-close-legs.js',
      './js/ui/journal-trade-forms.js', './js/ui/journal-trade-detail.js',
      './js/ui/backend-candle-store-chart.js', './js/ui/chart-interactions.js'].slice().sort(),
     'SCOPE: js/ui/ contributes exactly twenty named scripts, including the later Journal UI, Backup/Restore, MCX macro-check, MCX charts, TT reconnect, Journal Close Legs and trade-forms owners');
