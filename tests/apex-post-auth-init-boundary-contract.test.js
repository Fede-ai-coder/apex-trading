...
eq(LIVE_INDEX.length, JOURNAL_MAP_AUDIT_U.EXTRACTED_CHARS, 'the live shipped index UTF-16 length is the newest layer’s extracted value');
eq(sha256(LIVE_INDEX), JOURNAL_MAP_AUDIT_U.EXTRACTED_SHA256, 'the live shipped index SHA-256 is the newest layer’s extracted value');
...
// nothing.
eq(PRE_VEGA_MONITOR.length, STRATEGY_TEMPLATES_U.EXTRACTED_CHARS,
  'peeling the vega monitor reaches the strategy-templates extracted length');
eq(sha256(PRE_VEGA_MONITOR), STRATEGY_TEMPLATES_U.EXTRACTED_SHA256, '…and its hash');
...
// The same re-termination, one cycle earlier: this pins the post-snapshot-prefetch
// document that the DXLink-greeks lines above used to reach.
...
eq(sha256(PRE_RICH_SNAPSHOT), CANDLE_CHART_U.EXTRACTED_SHA256,
  '…and its hash');
eq(localScripts(LIVE_INDEX).length, 80, 'the live shipped index carries 80 local application scripts');
...
eq(APP_PARTS.map((p) => p.src || '(inline)').slice(-27),
...
ok(changed.every((rel) => rel === 'index.html' || rel === 'js/portfolio/portfolio-vega-monitor.js' || rel === MODULE_REL ||
    ...
    rel === 'js/services/journal-map-audit.js' || rel === 'CLAUDE.md' ||
