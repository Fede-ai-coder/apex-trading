...
eq(LIVE_INDEX.length, SNAPSHOT_PREFETCH_U.EXTRACTED_CHARS, 'the live shipped index UTF-16 length is the newest layer’s extracted value');
eq(sha256(LIVE_INDEX), SNAPSHOT_PREFETCH_U.EXTRACTED_SHA256, 'the live shipped index SHA-256 is the newest layer’s extracted value');
// Re-terminate the chain: the backend-candles document these two lines used
// to pin is now one layer down, so it is pinned explicitly here.
eq(PRE_SNAPSHOT_PREFETCH.length, BACKEND_CANDLES_U.EXTRACTED_CHARS,
  'peeling the snapshot prefetch reaches the backend-candles extracted length');
eq(sha256(PRE_SNAPSHOT_PREFETCH), BACKEND_CANDLES_U.EXTRACTED_SHA256, '…and its hash');
...
eq(PRE_RICH_SNAPSHOT.length, CANDLE_CHART_U.EXTRACTED_CHARS,
...
eq(sha256(PRE_RICH_SNAPSHOT), CANDLE_CHART_U.EXTRACTED_SHA256,
  '…and its hash');
eq(localScripts(LIVE_INDEX).length, 67, 'the live shipped index carries 67 local application scripts');
...
eq(APP_PARTS.map((p) => p.src || '(inline)').slice(-14),
...
ok(changed.every((rel) => rel === 'index.html' || rel === MODULE_REL || rel === 'CLAUDE.md' ||
  rel === 'js/ui/tt-reconnect.js' || rel === 'js/ui/journal-close-legs.js' || rel === 'js/portfolio/portfolio-expiry-manual.js' || rel === 'js/portfolio/portfolio-traffic-light.js' || rel === 'js/ui/backend-candle-store-chart.js' || rel === 'js/services/journal-rich-snapshot.js' || rel === 'js/portfolio/portfolio-backend-candles.js' || rel === 'js/services/journal-snapshot-prefetch.js' || rel === 'js/portfolio/backend-portfolios.js' || rel === 'js/portfolio/portfolio-data-fetch.js' || rel === 'js/ui/journal-trade-detail.js' || rel === 'js/ui/journal-trade-forms.js' || rel.startsWith('tests/')),
