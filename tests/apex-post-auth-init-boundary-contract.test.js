...
eq(LIVE_INDEX.length, SCANNER_EARNINGS_U.EXTRACTED_CHARS, 'the live shipped index UTF-16 length is the newest layer’s extracted value');
eq(sha256(LIVE_INDEX), SCANNER_EARNINGS_U.EXTRACTED_SHA256, 'the live shipped index SHA-256 is the newest layer’s extracted value');
...
eq(sha256(PRE_RICH_SNAPSHOT), CANDLE_CHART_U.EXTRACTED_SHA256,
  '…and its hash');
eq(localScripts(LIVE_INDEX).length, 72, 'the live shipped index carries 72 local application scripts');
...
ok(changed.every((rel) => rel === 'index.html' || rel === 'js/portfolio/portfolio-vega-monitor.js' || rel === MODULE_REL ||
    ...
    rel === 'js/services/scanner-earnings-throttle.js' || rel === 'CLAUDE.md' ||
