...
eq(LIVE_INDEX.length, PORTFOLIO_TECHNICAL_MERGE_U.EXTRACTED_CHARS, 'the live shipped index UTF-16 length is the newest layer’s extracted value');
eq(sha256(LIVE_INDEX), PORTFOLIO_TECHNICAL_MERGE_U.EXTRACTED_SHA256, 'the live shipped index SHA-256 is the newest layer’s extracted value');
...
eq(sha256(PRE_RICH_SNAPSHOT), CANDLE_CHART_U.EXTRACTED_SHA256,
  '…and its hash');
eq(localScripts(LIVE_INDEX).length, 78, 'the live shipped index carries 78 local application scripts');
...
eq(APP_PARTS.map((p) => p.src || '(inline)').slice(-25),
