...
eq(sha256(PRE_RICH_SNAPSHOT), CANDLE_CHART_U.EXTRACTED_SHA256,
  '…and its hash');
eq(localScripts(LIVE_INDEX).length, 71, 'the live shipped index carries 71 local application scripts');
...
eq(APP_PARTS.map((p) => p.src || '(inline)').slice(-18),
