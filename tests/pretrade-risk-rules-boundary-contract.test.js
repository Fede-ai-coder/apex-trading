...
const allowedProduction=['index.html',MODULE_REL,TECHNICALS_REL,MODAL_REL,MCX_MODULE_REL,MCX_VIX_MODULE_REL,MCX_BACKEND_CANDLES_REL,JOURNAL_CORE_REL,'js/services/mcx-regime-policy.js','js/ui/journal-ui.js',JOURNAL_REMOTE_REL,JOURNAL_WRITE_THROUGH_REL,JOURNAL_MIGRATION_REL,JOURNAL_MANUAL_IMPORT_REL, 'js/ui/journal-backup-restore.js'];
const changedProduction=changed.filter(p=>p==='index.html'||p.startsWith('js/')).sort();
same(changedProduction,allowedProduction.slice().sort(),'production footprint is exactly index.html + all three stacked PRETRADE owners + all four MCX owners + seven Journal owners');
