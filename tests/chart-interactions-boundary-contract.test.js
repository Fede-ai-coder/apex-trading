...
// ─────────────────────────────────────────────────────────────────────────────
{
  ...
  eq(changed.filter((rel) => rel === 'index.html' || rel.startsWith('js/')),
    ['index.html', MODULE_REL, 'js/services/journal-snapshot-helpers.js', 'js/services/swing-weekly-candles.js', 'js/services/swing-direction.js', 'js/portfolio/backend-positions-aggregate.js', 'js/portfolio/portfolio-technical-merge.js', 'js/portfolio/portfolio-technical-alignment-debug.js'].sort(),
  ...
  // RETIRED IN #454, deliberately. The mutant budget's floor rises by one
  ...
  // layer #24's spec, #448 #25's, #450 both #26's and #27's, #452 #28's, and
  // this layer — #29 — is the next in that order. Every assertion in this
  // contract still runs on every push; what stopped is the mutation pass
  // re-proving those pins are load-bearing. Restoring it is a matter of writing
  // the spec again and raising DECLARED_MUTANTS by its count.
  ok(!fs.existsSync(path.join(ROOT, CONTRACT_SPEC_REL)),
    'this contract\'s mutation spec was RETIRED in #454 to hold the mutant budget flat');
