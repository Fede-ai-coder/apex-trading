...
// pinned, one layer down, by the preMcxCharts assertions above.
...
// by the preMcxMacroCheck assertions above.
...
eq(countLiteral(INDEX, BACKUP_MARKER), 0,
  'backup UI marker has zero inline residue now that Backup/Restore is its own owner');
eq(countLiteral(preBackupRestore, BACKUP_MARKER), 1,
  'backup UI marker was inline exactly once before the Backup/Restore extraction');
eq(countLiteral(BACKUP_RESTORE_MODULE, BACKUP_MARKER), 1,
  'backup UI marker now lives exactly once in the Backup/Restore owner');
...
, 'js/services/journal-map-audit.js', 'js/services/dxlink-greeks-fetch.js', 'js/portfolio/portfolio-technical-parity.js'].sort(), 'production footprint is index.html plus Write-through, Migration, Manual Import, Backup/Restore, MCX macro check, MCX charts, Apex post-auth, TT reconnect and Journal Close Legs');
