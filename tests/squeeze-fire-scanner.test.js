...
(function() {
  ...
  ok(/_sfsResolveRenderPrice/.test(drawFn), '_sfsDrawOneTf resolves one SFS render price');
  ok(/patchLastCandleWithLivePrice\(rawCandles,\s*_sfsRenderPrice\.price\)/.test(drawFn), '_sfsDrawOneTf patches with the resolved SFS render price');
  ok(/_sfsDrawRsPanel/.test(drawFn),  '_sfsDrawOneTf delegates RS rendering to the SFS read-only wrapper');
  var priceFn = extractFn(HTML, '_sfsResolveRenderPrice');
  ok(/resolveLatestDisplayPrice/.test(priceFn), '_sfsResolveRenderPrice uses the shared display-price resolver');
  ok(!/fetch\s*\(/.test(priceFn), '_sfsResolveRenderPrice is synchronous/no fetch');
  ok(!/_ensureCandleSubscription\s*\(/.test(priceFn), '_sfsResolveRenderPrice opens no Candle subscriptions');
