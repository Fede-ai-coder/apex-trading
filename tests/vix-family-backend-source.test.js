...
const logs = [];
const store = new Map();
let BACKEND_VIX; // per-test: object → returned; Error → thrown by ttCall; null → ok:false
const sandbox = {
...
function reset() { WS_INSTANCES.length = 0; logs.length = 0; store.clear(); sandbox.window = {}; sandbox.S.vixFamily = null; }
...
async function main() {
  ...
  section('1. backend VIX family is the primary source (no frontend websocket)');
  {
  ...
  section('7. static: backend-first wiring, endpoint, no Yahoo/new source');
  {
    ...
    ok(/fetchMarketContextVixFamilyFromBackend\(\)/.test(backendFirst), '7: backend-first calls the backend fetch first');
    ok(/fetchVixFamily\(\)/.test(backendFirst), '7: direct websocket retained as fallback');
    ok(backendFetch.includes("'/market-context/vix-family/live'"), '7: consumes GET /market-context/vix-family/live');
    ok(!/yahoo/i.test(backendFirst) && !/yahoo/i.test(backendFetch), '7: no Yahoo reference in backend path');
  }
