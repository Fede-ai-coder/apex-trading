...
//   repairs no defect. It reads the real post-PESS source at the audited base,
...
//   first relocation slice — which it also does not implement.
//
...
//   Every number in docs/refactoring/post-pess-monolith-extraction-audit.md is
...
//
//     regenerate:  AUDIT_WRITE_DOC=1 node tests/post-pess-monolith-extraction-audit.test.js
//     verify:      node tests/post-pess-monolith-extraction-audit.test.js
//
...
//   2. EIC — a duplicate declaration is not a blocker merely because it is
//      ugly. §7 analyses both duplicate pairs semantically, byte for byte, and
//      decides whether a relocation preserving BOTH sites is honest.
...
const DOC_PATH = path.join(ROOT, 'docs', 'refactoring', 'post-pess-monolith-extraction-audit.md');
...
// ═════════════════════════════════════════════════════════════════════════════
// §2  PROVE THE PARSER AGAINST ALL THREE COMPLETED FAMILIES  (audit phase 3)
//
...
const FAMILY_TOTALS = { DSB: [54, 48119], SFS: [62, 39822], PESS: [9, 52722] };

section('§2  PARSER PROVEN AGAINST THE THREE COMPLETED FAMILIES');

const moduleDecls = {};   // path -> declarations
const familyModuleNames = { DSB: [], SFS: [], PESS: [] };
...
eq(EXTERNAL_SCRIPTS.length, 33, 'external local application scripts on disk');
...
ok(DECLS.length > UNIQUE_NAMES.length, 'the monolith really does contain duplicate declaration sites');
...
//
//   • A completed family's pattern is near the top, so a residual would be
...
const RULES = [
  ...
  // Completed families. A match here is a RESIDUAL and fails §5.
  ...
  ['PRETRADE', /PreTrade/],
  ['PRETRADE', /^(computeFinalDecision|computeSetupScore|_fetchPretradeBackendCandles)$/],
...
// Families that are candidates for the NEXT extraction. Completed families are
// excluded structurally, not by a filter someone can forget to apply.
...
// ═════════════════════════════════════════════════════════════════════════════
// §5  COMPLETED-FAMILY RESIDUAL PROOF  (audit phases 3 + 20)
//
// Two independent proofs, because they can fail for different reasons:
//   A. NAME PROOF — not one of the 125 names declared by the DSB/SFS/PESS
...
//   B. CLASSIFIER PROOF — no inline declaration classifies as a completed
...
section('§5  DSB / SFS / PESS RESIDUAL PROOF');
...
const OLD_NOT_FOUND = OLD_369_LIST.filter((n) => FOUND_NAMES.indexOf(n) < 0);
...
ok(DUPLICATES.length > 0, 'the duplicate analysis actually ran on real duplicates');
...
eq(CONFLICTS.EIC.category, 'LOAD_ORDER', 'EIC — no PR touches any EIC body; only #362 script tags');
...
ok(CONFLICTS.EIC.prs.length === 1 && CONFLICTS.EIC.prs[0] === 362,
  'EIC — the single overlapping PR is #362 and only in the script-tag region');
...
for (const k of Object.keys(EXEC_WEIGHTS)) {
  for (const delta of [-0.2, 0.2]) {
    ...
    const r = scoreAll(CANDIDATES, perturb(EXEC_WEIGHTS, factors), 'exec').filter((x) => !GATED[x.name].length);
    SENSITIVITY.singleWeight.push({ weight: k, delta: delta, winner: r[0].name });
  }
}
...
for (let t = 0; t < TRIALS; t++) {
  ...
  SENSITIVITY.winnerFrequency[elig[0].name] = (SENSITIVITY.winnerFrequency[elig[0].name] || 0) + 1;
  ...
  for (let pct = 1; pct <= 60; pct++) {
    for (const k of Object.keys(EXEC_WEIGHTS)) {
      for (const sign of [-1, 1]) {
        ...
        const r = scoreAll(CANDIDATES, perturb(EXEC_WEIGHTS, factors), 'exec').filter((x) => !GATED[x.name].length);
        if (r[0].name !== WINNER.name) {
          SENSITIVITY.smallestFlip = { weight: k, deltaPct: sign * pct, newWinner: r[0].name };
          return;
        }
      }
    }
...
//
// FOUR, not three: the post-PESS field is close and the fourth place is worth
...
// ═════════════════════════════════════════════════════════════════════════════
// §14  SPECIAL EIC DEEP AUDIT  (audit phase 15)
//
// EIC must also earn approval or rejection. The previous audit recorded 11
// declaration sites / 9 unique names / 2 duplicate declarations / ~67,352 chars
// and left the duplication unresolved. This section settles it.
// ═════════════════════════════════════════════════════════════════════════════
...
eq(EIC.sites, 11, 'EIC — declaration sites');
eq(EIC.uniqueNames, 9, 'EIC — unique declaration names');
eq(EIC.duplicateSites, 2, 'EIC — duplicate declaration sites');
eq(EIC.chars, 67352, 'EIC — declaration chars (matches the previously reported ~67,352)');
eq(EIC.runs, 1, 'EIC — ONE physical run: all 11 sites are consecutive');
...
eq(EIC.stateWrites, ['eicShowAll'], 'EIC — writes exactly one shared-state field');
eq(EIC.globals, 0, 'EIC — exposes nothing on window');

for (const d of EIC_DUPS) {
  ok(d.identicalBytes, d.name + ' — the two sites are BYTE-IDENTICAL');
  ok(d.sameBindingForm, d.name + ' — both sites use the same binding form (' + d.bindingForms.join('/') + ')');
  ok(d.hoisted, d.name + ' — both sites are hoisted function declarations, bound before statement one runs');
  eq(d.interveningTopLevelStatements, 0, d.name + ' — no top-level statement executes between the two sites');
  eq(d.classification, 'RELOCATION_SAFE_BOTH_SITES', d.name + ' — verdict');
}
eq(EIC_DUPS.length, 2, 'EIC — exactly two duplicated names analysed');
eq(uniq(EIC_DUPS.map((d) => d.name)).sort(), ['eicFetchLegs', 'eicLiqFromLegs'], 'EIC — the duplicated names');

const EIC_VERDICT = {
  blocked: false,
  duplicationBlocks: false,
  reason: 'RELOCATION-SAFE. Both duplicated names are byte-identical `function` declarations. In a classic '
    + 'script both sites are bound at HOIST time, before any statement executes, so the later site wins and — '
    + 'being byte-identical — wins with the same function object shape the earlier one would have produced. '
    + 'Nothing executes between the sites, and nothing between them references the names. Moving BOTH sites '
    + 'into one module, preserving their relative order, is a byte-for-byte semantics-preserving relocation. '
    + 'No deduplication is required, and none may be performed inside an extraction PR.',
};
ok(!EIC_VERDICT.blocked, 'EIC — VERDICT: not blocked by duplication');
...
//
// Five options, compared on the same measurements. Two of them turn out not to
// EXIST for this family, and saying so is a finding rather than an omission:
// EIC has no `var`/`const`/`let` declaration at all — all eleven sites are
// function declarations — so any option whose first module is "state/config"
...
const G_PURE = pick(['eicScreenTicker', 'eicLiqFromLegs', 'eicBuildLiveContext']);
const G_PANEL = pick(['runEICPanel', 'eicAnalyzeAll']);
const G_TICKER = pick(['eicAnalyzeTicker']);
const G_LIVE = pick(['eicFetchLegs', 'eicDXLinkDeepDive', 'eicRunDXLink']);
...
const SPLIT_OPTIONS = [
  {
    id: 'A', label: 'one module', available: true,
    modules: [{ path: 'js/ui/eic-panel.js', decls: EIC.declarations.length, chars: EIC.chars, owner: 'everything' }],
    prs: 1,
  },
  {
    id: 'B', label: 'state/config + rest', available: false,
    unavailableBecause: 'EIC declares no var/const/let at all — all 11 sites are function declarations, so the state/config module would be empty',
    modules: [], prs: 0,
  },
  {
    id: 'C', label: 'service + UI', available: true,
    modules: [
      { path: 'js/services/eic-screening-rules.js', decls: EIC_ZERO_EFFECT.length, chars: charsOf(EIC_ZERO_EFFECT), owner: 'pure screening + liquidity + live-context rules' },
      { path: 'js/ui/eic-panel.js', decls: EIC_EFFECTFUL.length, chars: charsOf(EIC_EFFECTFUL), owner: 'panel, analysis-and-render, live transport' },
    ],
    prs: 2,
  },
  {
  ...
  {
    id: 'E', label: 'source-derived: rules · panel · ticker analysis · live deep dive', available: true,
    modules: [
      { path: 'js/services/eic-screening-rules.js', decls: G_PURE.length, chars: charsOf(G_PURE), owner: 'pure rules — zero DOM, zero network, zero state, zero timers, zero listeners' },
      { path: 'js/ui/eic-panel.js', decls: G_PANEL.length, chars: charsOf(G_PANEL), owner: 'panel shell + batch orchestration; renders its own results' },
      { path: 'js/ui/eic-ticker-analysis-panel.js', decls: G_TICKER.length, chars: charsOf(G_TICKER), owner: 'single-ticker analysis that renders its own cards — analysis AND render, named as such' },
      { path: 'js/ui/eic-live-deep-dive.js', decls: G_LIVE.length, chars: charsOf(G_LIVE), owner: 'live DXLink acquisition that also renders — transport AND render, named as such' },
    ],
    prs: 4,
  },
];

const RECOMMENDED_SPLIT = 'E';
const CHOSEN = SPLIT_OPTIONS.find((o) => o.id === RECOMMENDED_SPLIT);

// Every option that claims to be available must account for all 11 sites.
...
eq(EIC_STATE_DECLS.length, 0, 'EIC declares no var/const/let — options B and D genuinely do not exist');
for (const o of SPLIT_OPTIONS.filter((x) => !x.available)) {
...
for (const o of SPLIT_OPTIONS.filter((x) => x.available)) {
  for (const m of o.modules) {
    ...
    const contents = o.id === 'E' && m.path.indexOf('screening-rules') >= 0 ? G_PURE
      : o.id === 'C' ? EIC_ZERO_EFFECT : [];
...
for (const m of CHOSEN.modules) {
  const group = m.path.indexOf('screening-rules') >= 0 ? G_PURE
    : m.path.indexOf('eic-panel') >= 0 ? G_PANEL
    : m.path.indexOf('ticker-analysis') >= 0 ? G_TICKER : G_LIVE;
...
ok(CEILING_REPORT.find((c) => c.option === 'A').maxModule > ADVISORY_CEILING,
  'option A (one module) is ABOVE the advisory ceiling — reported, not forbidden');

// ═════════════════════════════════════════════════════════════════════════════
// §17  THE FIRST SLICE  (audit phase 18)
//
// PROPOSED ONLY. Nothing here is implemented, and this audit must not implement
// it. The slice is the most inert owner EIC has: four declaration sites that
...
const FIRST_SLICE = {
  module: 'js/services/eic-screening-rules.js',
  ...
  expectedScriptPosition: 'anywhere before the inline monolith; it has NO evaluation-time dependency on anything. '
    + 'Natural home is the family/module region alongside js/ui/pess-panel.js.',
};

eq(FIRST_SLICE.chars, 14368, 'first slice — total chars');
eq(FIRST_SLICE.sites, 4, 'first slice — declaration sites');
eq(FIRST_SLICE.uniqueNames, 3, 'first slice — unique names');
eq(FIRST_SLICE.duplicateSites, 1, 'first slice — carries one duplicate pair (eicLiqFromLegs)');
eq(FIRST_SLICE.physicalOrder, ['eicScreenTicker', 'eicLiqFromLegs', 'eicLiqFromLegs', 'eicBuildLiveContext'],
  'first slice — original physical order is preserved, both duplicate sites included');
...
const EIC_DUP_STILL_PRESENT = EIC_DUPS.length === 2 && EIC_DUPS.every((d) => d.identicalBytes);
DEFECTS.push({
  area: 'EIC', id: 'duplicate declarations (eicFetchLegs, eicLiqFromLegs)',
  status: EIC_DUP_STILL_PRESENT
    ? 'STILL PRESENT — 2 duplicated names, 2 extra sites, both byte-identical. NOT a blocker (§14). NOT fixed here, '
      + 'and must NOT be deduplicated inside an extraction PR.'
    : 'no longer present',
});
...
function buildReport() {
  ...
  P('# Post-PESS conflict-aware monolith extraction audit');
  P();
  P('> **AUDIT ONLY — NO EXTRACTION.** Nothing was extracted, relocated, deduplicated or repaired.');
  P('> This document is generated by `tests/post-pess-monolith-extraction-audit.test.js`.');
  P('> Regenerate with `AUDIT_WRITE_DOC=1 node tests/post-pess-monolith-extraction-audit.test.js`.');
  ...
  P('| | |');
  P('|---|---|');
  P('| Audited base | `' + BASE_SHA + '` (origin/dev-clean, merge of PR #373) |');
  ...
  P();
  P('## 2. Parser proven against the completed families');
  P();
  ...
  P();
  P('`Est. PRs` is the COARSE SCORING PROXY only — `ceil(chars / ' + n(ADVISORY_CEILING) + ')`. It feeds the');
  P('`completability` factor and nothing else. The winner\'s ACTUAL recommended PR count comes from its');
  P('source-derived split in §15 and is larger: the proxy asks "how many ceiling-sized modules of bytes is');
  P('this?", the split asks "where do the ownership boundaries actually fall?". Where they disagree, the');
  P('split is the answer.');
  P();
  ...
  P();
  P('Sensitivity never promotes a gated candidate: the gates are re-applied inside every trial.');
  P();
  ...
  for (const name of TOP_FOUR) {
    ...
    P('- called in from: ' + (Object.keys(m.calledInFrom).length ? Object.keys(m.calledInFrom).map((k) => k + ' (' + m.calledInFrom[k].join(', ') + ')').join(', ') : 'nothing'));
  ...
  P('| | Previously reported | Measured now |');
  P('|---|---:|---:|');
  P('| Declaration sites | 11 | ' + EIC.sites + ' |');
  P('| Unique names | 9 | ' + EIC.uniqueNames + ' |');
  P('| Duplicate sites | 2 | ' + EIC.duplicateSites + ' |');
  P('| Declaration chars | ~67,352 | ' + n(EIC.chars) + ' |');
  P();
  P('**' + EIC_VERDICT.reason + '**');
  P();
  P('EIC is therefore **not blocked** and becomes **relocation-safe**.');
  P();
  ...
  P();
  P('EIC wins execution priority **after** both hard gates, not before them:');
  P();
  P('- **Ownership gate — passes at grade ' + FAMS.EIC.ownership + '.** ' + FAMS.EIC.ownershipReasons.join('; ') + '.');
  P('  The last clause is the one point it LOSES, and it is the reason the recommended split below refuses');
  P('  to file its large bodies under `services/`.');
  ...
  P('- It is the only candidate with **one physical run** — all ' + EIC.sites + ' sites are consecutive, no other');
  P('  family\'s declaration sits inside its span, and no top-level statement is interleaved.');
  P('- **Zero** foreign writes in either direction; it writes exactly one shared-state field, `S.eicShowAll`;');
  P('  it exposes nothing on `window`.');
  P('- It is the modal winner in ' + n(SENSITIVITY.winnerFrequency[WINNER.name]) + '/' + n(SENSITIVITY.randomTrials)
    + ' randomised reweightings and does not flip under any single-weight move within ±60%.');
  P();
  P('One caveat is recorded rather than hidden: EIC has **' + EIC.tests.length + '** existing test suite(s)');
  P('referencing any of its declarations. That is the same position PESS was in when it was chosen, and the');
  P('same conclusion follows — the extraction must bring its own boundary contract, because nothing else in');
  P('the suite would notice a mistake.');
  P();
  ...
  P();
  P('**Recommended: option ' + RECOMMENDED_SPLIT + ' — ' + CHOSEN.prs + ' modules, ' + CHOSEN.prs + ' PRs.**');
  P();
  P('This is ' + CHOSEN.prs + ' PRs, not the ' + FAMS.EIC.likelyPRs + ' the §9 ceiling proxy estimated. The proxy');
  P('divides bytes by the advisory ceiling; the split follows where ownership actually changes. Two of these');
  P('four modules would fit inside one ceiling-sized file and are still kept apart, because merging them');
  P('would put effect-free rules in the same module as a body that opens a WebSocket.');
  P();
  ...
  P();
  P('Module names are justified from the bodies, not from a tidy-sounding layer cake. Only');
  P('`eic-screening-rules.js` is filed under `services/`, and every body in it is effect-free — zero DOM,');
  P('zero network, zero timers, zero listeners, zero state. The other three render, so none of them is called');
  P('a service: `eic-ticker-analysis-panel.js` and `eic-live-deep-dive.js` are named for what they actually');
  P('are — analysis-and-render and transport-and-render respectively. This is the `pessAnalyzeAll` correction');
  P('applied in advance rather than after the fact.');
  P();
  ...
  P('Target module: `' + FIRST_SLICE.module + '` · ' + FIRST_SLICE.sites + ' declaration sites · '
    + FIRST_SLICE.uniqueNames + ' unique names · ' + FIRST_SLICE.duplicateSites + ' duplicate pair · '
  ...
  P();
  P('- **Evaluation-time dependencies:** none. All four sites are plain `function` declarations; nothing');
  P('  evaluates at load time.');
  P('- **Call-time dependencies:** none. Not one of the four calls any other top-level declaration.');
  ...
  P('- **Duplicate handling:** both `eicLiqFromLegs` sites move together, in their original relative order.');
  P('  Neither is deleted. Deduplication is explicitly out of scope for any extraction PR.');
  P('- **Expected script position:** ' + FIRST_SLICE.expectedScriptPosition);
  P();
  P('This is the most inert owner EIC has, which is why it goes first — but the ordering rule is ownership');
  P('integrity, not slice size.');
  P();
...
const BASE_SHA = '73ecb1683d916a9eb2521654d88c530907e6462b';
const BASE_TREE = '7cb6238536729bba75058f4b9a18b90a178d426a';
const RECOVERY_BRANCH = 'backup/dev-clean-post-pess-complete-pre-next-monolith-audit-2026-08-16';
...
ok(DOC_EXISTS, 'the generated report exists at docs/refactoring/post-pess-monolith-extraction-audit.md');
if (DOC_EXISTS) {
...
mutant('parser', 'duplicate declaration missed', () => {
  // A parser that only reported the FIRST site of each name.
  const seen = new Set(), mutated = [];
  for (const d of DECLS) { if (seen.has(d.name)) continue; seen.add(d.name); mutated.push(d); }
  // Guard: sites must exceed unique names.
  return !(mutated.length > new Set(mutated.map((d) => d.name)).size);
});
mutant('parser', 'duplicate sites collapsed into one', () => {
  const collapsed = uniq(DECLS.map((d) => d.name)).map((nm) => DECLS.find((d) => d.name === nm));
  const dupExtra = collapsed.length - new Set(collapsed.map((d) => d.name)).size;
  // Guard: the EIC deep audit asserts exactly 2 duplicate sites.
  return dupExtra !== EIC.duplicateSites;
});
...
mutant('ownership', 'declaration assigned wrong family', () => {
  // Move the EIC pattern BELOW the generic PORTFOLIO topic rules, so EIC
  // declarations get absorbed elsewhere.
  const bad = RULES.filter((r) => r[0] !== 'EIC').concat(RULES.filter((r) => r[0] === 'EIC'));
  const inv = DECLS.map((d) => ({ name: d.name, family: classifyName(d.name, bad) }));
  const eicCount = inv.filter((d) => d.family === 'EIC').length;
  // Guard: the EIC deep audit pins 11 sites.
  return eicCount !== EIC.sites;
});
...
mutant('ownership', 'foreign write ignored (shadow filter disabled)', () => {
  // Without shadow awareness, `var ivrColor = …` inside runEICPanel counts as
  // EIC writing PORTFOLIO's binding.
  const s = stripLiterals(INV.find((d) => d.name === 'runEICPanel').text);
  const naive = uniq((s.match(/(?:^|[^.\w$])([A-Za-z_$][A-Za-z0-9_$]*)\s*(?:=[^=>]|\+\+|--|\+=|-=)/g) || [])
    .map((x) => { const m = x.match(/([A-Za-z_$][A-Za-z0-9_$]*)\s*(?:=|\+\+|--|\+=|-=)/); return m ? m[1] : null; })
    .filter((nm) => nm && TOP_NAMES.has(nm)));
  const real = INV.find((d) => d.name === 'runEICPanel').bindingWrites;
...
mutant('ownership', 'DOM-owning declaration mislabelled a service', () => {
  const fake = { path: 'js/services/eic-analysis-service.js', group: G_TICKER };
  const dirty = fake.group.filter((d) => d.dom > 0 || d.network > 0 || d.timers > 0 || d.listeners > 0);
...
mutant('ownership', 'mixed UI body given a false zero-DOM claim', () => {
  const d = INV.find((x) => x.name === 'eicAnalyzeTicker');
...
mutant('ownership', 'EIC duplicate auto-marked blocker without semantic analysis', () => {
  const lazy = DUPLICATES.map((x) => Object.assign({}, x, { classification: 'SEMANTIC_BLOCKER', relocationSafe: false }));
  const lazyEic = lazy.filter((x) => x.family === 'EIC');
  // Guard: §14 asserts RELOCATION_SAFE_BOTH_SITES for both, from byte equality.
  return lazyEic.every((x) => !x.relocationSafe) && EIC_DUPS.every((x) => x.relocationSafe) && EIC_DUPS.every((x) => x.identicalBytes);
});
...
mutant('conflict', 'blocked family promoted to winner', () => {
  ...
  const gatedWinnerWouldBe = GATED[noGate.name] && GATED[noGate.name].length > 0;
  // Guard: either the top raw scorer is ungated (and equals the winner), or the
  // gate demoted it — and the winner must be ungated either way.
  return GATED[WINNER.name].length === 0 && (!gatedWinnerWouldBe ? noGate.name === WINNER.name : noGate.name !== WINNER.name);
});
...
mutant('ranking', 'conflict gate disabled', () => {
  ...
  const withGate = ELIGIBLE[0];
  const anyGated = CANDIDATES.some((c) => conflictGateBlocks(c));
  return anyGated && withGate.name === WINNER.name && conflictGateBlocks(FAMS.SWING);
});
...
console.log('  recommended split    option ' + RECOMMENDED_SPLIT + ' — ' + CHOSEN.prs + ' modules, ' + CHOSEN.prs + ' PRs');
